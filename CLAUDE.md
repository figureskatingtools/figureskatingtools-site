...
- **`infra/hovtp`** — the **HOVTP listener**, a *separate* Function App (`func-fs-hovtp-*`, FC1 Python 3.13, own `asp-fs-hovtp` plan and `app-package-hovtp` container on the shared platform storage, shares `ai-fs-platform`) so the platform's only anonymous internet-facing surface can be stopped or revoked on its own. FS Manager pushes ODF/PDF messages to `POST|OPTIONS /api/hovtp[/{code}]` (no credentials exist in the protocol). Parsing is **body-first** — the competition code comes from the path, else `OdfBody/@CompetitionCode`; HOVTP headers are optional. Only `DT_PDF`, `DT_PARTIC`, `DT_PARTIC_TEAMS`, `DT_SCHEDULE`, `DT_SCHEDULE_UPDATE` are stored (`HOVTP_ALLOWED_DOCUMENT_TYPES` overrides); anything else is logged and dropped with a 200. Trust is per **(competition, source IP)**: table rows `PK=HOVTPSOURCE`, sessions/serial numbers in `PK=HOVTPSESSION`. An unaccepted IP's files are quarantined in `competition-data/<guid>/fsm-pending/<ip>/` (invisible in the file pool) and moved flat into `<guid>/fsm/` when an operator accepts the source. `HOVTP_ENABLED=false` is the kill switch (Bicep-owned, so it survives redeploys; GitHub var `HOVTP_ENABLED`). Own pytest suite — see Common commands.
...
`.github/workflows/deploy-site.yml` — push to `main` → prod; `test` only via `workflow_dispatch`. Five jobs: `set-environment` → `deploy-infra` (bicep + FIC/redirect-URI Graph step) → `deploy-platform-backend` (pytest, then zip deploy) → `deploy-hovtp-backend` (same shape for `infra/hovtp`, plus a warning-only `OPTIONS /api/hovtp` smoke check) → `deploy-frontend` (builds + router tests, stages `server/server.js` + `site/dist/*` → `public/`, async zip deploy + Kudu poll, `/health` smoke check).
