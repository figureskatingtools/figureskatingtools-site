...
- **`infra`** — subscription-scoped Bicep: per-env site RG (`rg-fs-site-<env>`) with the Web App (B1, Easy Auth v2 via MI federated credential), platform storage (`stfsplat*`: `competition-data` + `app-package` containers, `competitions` table), platform Function App (`func-fs-platform-*`, FC1 Python 3.13), the HOVTP listener's **own** storage account (`stfshovtp*`: host state + its `app-package`), RBAC (`platform-roleassignment.bicep` account-scoped for the platform app; `hovtp-host-roleassignment.bicep` account-scoped on the listener's own account; `hovtp-data-access.bicep` granting the listener *only* the `competition-data` container + the `competitions` table on platform storage; `shared-data-access.bicep` granting tool Function Apps Blob Data Reader on platform storage via `TOOL_PRINCIPAL_ID_*`), DNS in the shared persistent `rg-fs-dns` zone (apex = plain **A record to the Web App's `inboundIpAddress`** + `asuid` TXT; `test` = CNAME + `asuid.test` TXT), custom-domain binding + managed cert + SNI. **`infra/functions/`** is the platform competition API. **`infra/MIGRATION.md`** documents the one-time manual steps (GitHub env vars/secrets, Entra grants, cutover order, teardown).
- **`infra/hovtp`** — the **HOVTP listener**, a *separate* Function App (`func-fs-hovtp-*`, FC1 Python 3.13, own `asp-fs-hovtp` plan and its **own storage account** `stfshovtp*` for Functions host state + deployment package, shares `ai-fs-platform`) so the platform's only anonymous internet-facing surface can be stopped or revoked on its own. FS Manager pushes ODF/PDF messages to `POST|OPTIONS /api/v1/hovtp` (no credentials exist in the protocol; that single versioned route is what goes in FSM's Endpoint field). Parsing is **body-first** — the competition code comes from `OdfBody/@CompetitionCode`, falling back to an `X-*-Competition*` header; the URL never carries a code, and HOVTP headers are optional. Only `DT_PDF`, `DT_PARTIC`, `DT_PARTIC_TEAMS`, `DT_SCHEDULE`, `DT_SCHEDULE_UPDATE` are stored (`HOVTP_ALLOWED_DOCUMENT_TYPES` overrides); anything else is logged and dropped with a 200. Trust is per **(competition, source IP)**: table rows `PK=HOVTPSOURCE`, sessions/serial numbers in `PK=HOVTPSESSION`. An unaccepted IP's files are quarantined in `competition-data/<guid>/fsm-pending/<ip>/` (invisible in the file pool) and moved flat into `<guid>/fsm/` when an operator accepts the source. `HOVTP_ENABLED=false` is the kill switch (Bicep-owned, so it survives redeploys; GitHub var `HOVTP_ENABLED`). **Test is published through a Front Door Premium + WAF -> APIM layer** (`https://test-api.figureskatingtools.com/api/v1/hovtp`, Terraform in the separate `azure-publishing` repo): that makes `X-Forwarded-For` four entries long with FSM first, so `HOVTP_TRUSTED_PROXY_HOPS=3` there (`0` in prod, which is still direct), and because hop counting is spoofable while the raw `*.azurewebsites.net` hostname stays reachable, APIM injects `X-Proxy-Secret` — compared constant-time against `PROXY_SHARED_SECRET` (GitHub secret `PROXY_SHARED_SECRET_HOVTP`), `403` on a mismatch for POST/OPTIONS, fails open when unset, `/api/health` never gated. Storage is **isolated**: account-scoped roles only on its own `stfshovtp*` account (the Functions host creates `azure-webjobs-*` itself, which container-scoped RBAC cannot), and on the platform account nothing but container-scoped `competition-data` + table-scoped `competitions` — so it cannot reach the platform app's `app-package` deployment zip. `COMPETITION_DATA_ACCOUNT` names the platform account, because `AzureWebJobsStorage__accountName` no longer does (`_data_account_name()`, kept byte-identical with the platform app by `tests/test_parity.py`). Own pytest suite — see Common commands.
...
npm run dev --workspace=site                               # Vite MPA dev server (no API proxies — use the router for a full stack)
...
cd infra/hovtp && uv run --with-requirements requirements.txt \
  --with-requirements requirements-dev.txt python -m pytest tests -q   # HOVTP listener suite
```
...
`.github/workflows/deploy-site.yml` — push to `main` → prod; `test` only via `workflow_dispatch`. Five jobs: `set-environment` → `deploy-infra` (bicep + FIC/redirect-URI Graph step) → `deploy-platform-backend` (pytest, then zip deploy) → `deploy-hovtp-backend` (same shape for `infra/hovtp`, plus a warning-only `OPTIONS /api/v1/hovtp` smoke check) → `deploy-frontend` (builds + router tests, stages `server/server.js` + `site/dist/*` → `public/`, async zip deploy + Kudu poll, `/health` smoke check). Dependabot version updates (`.github/dependabot.yml`, read from `main` only) target `test` and ride the next `test` → `main` promotion instead of auto-deploying prod on merge; GitHub's security-update PRs still target `main` and may be merged directly.

Per-environment GitHub config: vars `FUNCTION_APP_URL_{JUDGEPAPERS,SCOREMODIFIER,PROTOCOLGENERATOR}`, `TOOL_PRINCIPAL_ID_{…}`, `SKIP_CUSTOM_DOMAIN` (set `true` to run on the default hostname, e.g. pre-cutover), `HOVTP_ENABLED` (optional; `false` = the deployed HOVTP listener answers every request 503 — defaults are on in test, off in prod); secrets `PROXY_SHARED_SECRET_{JUDGEPAPERS,SCOREMODIFIER,PROTOCOLGENERATOR,PLATFORM}`, `PROXY_SHARED_SECRET_HOVTP` (test only — the value APIM injects, from the `azure-publishing` repo), `AUTH_CLIENT_ID`, `AUTH_APP_OBJECT_ID`. Bicepparams read secrets via `readEnvironmentVariable()`. `deploy-infra` is **atomic**: `webapp.bicep` still replaces the whole app-settings collection, but it now writes the full set itself (`FUNCTION_APP_URL_*` — platform from the platform Function App module's output, the tools from the GitHub env config passed into the template — plus `PROXY_SHARED_SECRET_*` and `SCM_DO_BUILD_DURING_DEPLOYMENT`), so running it alone leaves the router fully configured.

`publish-shared-ui.yml` is **manual-dispatch only** (the push trigger 409'd on every main push touching shared-ui, since v3 stays at one version and is consumed via workspace symlink, not the registry); it exists solely to publish v2.x for the tool repos' legacy frontends — retire it (and mark shared-ui private) at teardown. Migration/cutover order, old-subdomain redirects and the teardown checklist live in `infra/MIGRATION.md`. All jobs in both workflows pin `runs-on: ubuntu-26.04` rather than `ubuntu-latest` ([actions/runner-images#14748](https://github.com/actions/runner-images/issues/14748) flips `-latest` from 24.04 to 26.04 between 2026-10-19 and 2026-11-19); bump that label deliberately — verify via a `test`-environment dispatch first, roll back to `ubuntu-24.04` if needed — and never set it back to `ubuntu-latest`.
